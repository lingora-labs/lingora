...
//
// KEY PRINCIPLE:
// The tutor DOES NOT generate PDFs.
// The system routes to deterministic generators.
//
// ARCH ALIGNMENT:
...
//
// OUTPUT:
// Uint8Array → Buffer → data URL (base64)
//
// =============================================================================

import { generateLessonPdf, LessonContent } from './pdf/generateLessonPdf';
import { generateCoursePdf, CourseContent } from './pdf/generateCoursePdf';

// ── Optional Storage Layer (S3 only if configured) ────────────────────────────
...
try {
  ...
  // No S3 → fallback handled automatically
}
  ...
  try {
    ...
    if (uploadToS3) {
      try {
        ...
        if (s3Url) {
          return { success: true, url: s3Url, method: 's3' };
        }
      } catch (err) {
        console.warn('[pdf-generator] S3 failed → fallback data URL', err);
      }
    }
...
// ── Fallback Generator (plain text export) ────────────────────────────────────
...
async function generatePlainTextPdf(title: string, content: string): Promise<Uint8Array> {
  ...
  page.drawText('lingora.netlify.app  ·  Exportado desde LINGORA', {
    x: 48,
    y: 9,
    size: 7.5,
    font: regular,
