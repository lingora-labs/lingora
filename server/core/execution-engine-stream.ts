  ...
  "generatedAt": "${now}"
        ...
        if (!courseContent) {
          const errorMsgs: Record<string, string> = {
            en: `Could not generate the course for "${topic}" right now. The content engine returned an error. Check the topic formulation.`,
            es: `No se pudo generar el curso sobre "${topic}" en este momento. El motor de contenido devolvio un error. Revisa la formulacion del tema.`,
            no: `Kunne ikke generere kurset om "${topic}" akkurat na. Innholdsmotoren returnerte en feil. Prøv en annen formulering av emnet.`,
          };
        ...
        if (!result.success) {
          const pdfErrorMsgs: Record<string, string> = {
            ...
            es: `El contenido del curso se genero pero el PDF no pudo renderizarse. Error: ${result.error ?? 'desconocido'}. Intenta con un tema mas corto.`,
            no: `Kursinnholdet ble generert, men PDF-en kunne ikke gjengis. Prøv en annen formulering.`,
          };
...
function getDegradedMentorMessage(lang: string, is429: boolean): string {
  // SEEK 4.1b — no placebo messages. Honest cause.
  if (is429) {
    const msgs: Record<string, string> = {
      en: 'The request could not be completed: API quota limit reached.',
      es: 'La solicitud no pudo completarse: límite de cuota de API alcanzado.',
      no: 'Forespørselen kunne ikke fullføres: API-kvotegrensen er nådd.',
    };
