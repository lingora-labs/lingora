// SEEK 5.0 P0 — types + static data extracted from archived beta page

export type MK = 'sarah' | 'alex' | 'nick'
export type TK = 'conversation' | 'structured' | 'cervantes' | 'business' | 'travel' | 'course' | 'leveltest'
export type Lang = 'es' | 'en' | 'no' | 'fr' | 'de' | 'it' | 'pt' | 'ar' | 'ja' | 'zh'

export const BACKEND_LANG = (l: Lang): string =>
  (['ar', 'ja', 'zh'] as Lang[]).includes(l) ? 'en' : l
export type Phase = 'onboarding' | 'splash' | 'mode' | 'chat'
...
export interface TableRow { left: string; right: string }
export interface QuizQ    { question: string; options: string[]; correct: number; explanation?: string }
export interface Sub      { title: string; content: string; keyTakeaway?: string }
...
export const MENTOR_META: Record<MK, { emoji: string; name: string; code: string; color: string; bg: string; spec: string }> = {
...
export const TOPIC_META: Record<TK, { emoji: string }> = {
...
export const LANG_GRID: Array<{ value: Lang; flag: string; label: string; sub: string }> = [
...
export const COPY: Record<Lang, { tg: string; l1: string; l2: string; l3: string; sb: string; lnw: string; hint: string; ph: string; tn: string[]; td: string[]; md: string[]; bio: string[] }> = {
...
export const TOPIC_KEYS: TK[] = ['conversation', 'structured', 'cervantes', 'business', 'travel', 'course', 'leveltest']
export const MENTOR_KEYS: MK[] = ['sarah', 'alex', 'nick']

export const GREETINGS: Record<MK, Partial<Record<Lang, string>>> = {
  sarah: {
...
export const TSYS: Record<TK, string> = {
